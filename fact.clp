...
* isWholeNumber - determines whether a given input is a whole number; if so, will cast the number to a long
*/
...
(deffunction fact (?n)
   ...
   (if (eq ?castedN FALSE) then (throw (new JessException "fact" "The input must be a whole number >= " 0)) ; routine, message, and additional data
...
/*
* Determines whether a given parameter (of any acceptable type) is a whole number. 
* If it is not, will return false; otherwise, will return the initial number casted to a long.
*/
(deffunction isWholeNumber (?n)
   ...
   (return ?returnVal) ; if ?n is not a whole number, false will be returned; if valid, will be casted to a long and returned
)
